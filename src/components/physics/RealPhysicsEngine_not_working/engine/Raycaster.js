...
class Raycaster {
    ...
    /**
     * Check if the point is inside a triangular face using the Same-Side Technique (https://blackpawn.com/texts/pointinpoly/)
    ...
    static pointInsideFace(point, face, coplanar = false) {
        ...
        if (!coplanar && _tmpVec1.copy(point).sub(face.midpoint).dot(face.normal) == 0) {
            return false;
        }

        // First side
        let side = _tmpVec1.subVectors(face.v2, face.v1);
        let vec = _tmpVec2.subVectors(point, face.v1);
        let inside = side.cross(vec).dot(face.normal);
        if (inside < 0) return false;

        // Second side
        side.subVectors(face.v3, face.v2);
        vec.subVectors(point, face.v2);
        inside = side.cross(vec).dot(face.normal);
        if (inside < 0) return false;

        // Third side
        side.subVectors(face.v1, face.v3);
        vec.subVectors(point, face.v3);
        inside = side.cross(vec).dot(face.normal);
        if (inside < 0) return false;
     ...
     */
    static raycastToFaces(lineOrigin, lineDirection, faces, length, firstHitOnly = true) {
        let intersections = [];
